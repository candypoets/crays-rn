...
- Scenario: `.qa/qa-account-recovery.mjs` owns clean bootstrap/teardown; its independent verifier requires exactly one expected public identity with `imported-privkey` custody, zero early profile/completion markers, and no nsec in logcat.
