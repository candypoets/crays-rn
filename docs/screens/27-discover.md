...
D-001 or a fabricated production listing. The named Test Room QA scenario
selects the next free local proxy port when 8787 belongs to another worktree
and carries that validated `ws:` or `wss:` address only through the
development-only identity seed route. When a direct link already targets the
test room's relay and id, the duplicate card and its subscription are suppressed
because relays replace a REQ that reuses a subscription ID.
