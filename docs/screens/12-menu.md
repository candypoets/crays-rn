...
- App shell header: plum mark, the signed room name as eyebrow, **Menu** title, tempo rail, and the cart pill (count and action) at right.
- **Back to room** return link.
- **Tonight's setlist** note: availability and prices come directly from the venue; payment methods appear at review. No live-set or kitchen-moment claim is rendered because no such data exists in props.
...
- Each product is a bright card: shared drink imagery for drink kinds (deterministic atlas index from product position; honest icon block otherwise), name, summary, exact formatted price, and a small blue add cue. Tapping a card opens screen 13 — it never places an order or implies kitchen state. Unavailable products stay visible but disabled and dimmed, never purchasable.
