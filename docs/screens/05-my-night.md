...
Canonical visual reference: `docs/design-explorations/night-playlist/mockups/04-durable-and-settings-v1.png`, panel 02. My Night uses the board's dark venue-photo prelude and calm white cue-sheet stack while the rest of the application remains on the pale-lilac Night Playlist canvas.
...
The first card overlaps the venue hero and renders the exact priority item. A valid access award may show a QR-shaped credential cue, but the actual signed presentation is generated only after **Show at the door** opens the ticket route. Active order and relevant membership follow as stable white cards with text status in addition to signal color. Internal order references never appear.
