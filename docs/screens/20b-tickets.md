...
through the public UI, and opens Tickets. `scenario:20b-tickets`
independently verifies the exact calendar address, accepted status, signer
signature, room-definition consumption, and teardown. `scenario:20c-ticket-detail`
...
and revoked/ended records. `scenario:20d-rsvp-rejected` owns the rejection
