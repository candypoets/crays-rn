...
function fakeManager(authPubkey: string, account?: { type: string; payload: unknown }) {
  ...
  return {
    ...
    getAccounts: jest.fn(() => account ? { [authPubkey]: account } : {}),
    ...
    setNip46QR: jest.fn(),
    setPubkey: jest.fn(),
    setSigner: jest.fn(() => queueMicrotask(() => {
      active = authPubkey;
...
describe('existing Nostr identity access', () => {
  ...
  it('imports an nsec only after signer ownership is confirmed, then stores it protected', async () => {
    ...
    expect(SecureStore.setItemAsync).toHaveBeenCalledWith('crays.identity.nsec', nsec, expect.any(Object));
    expect(SecureStore.setItemAsync).toHaveBeenCalledWith(
      'crays.identity.signer',
      JSON.stringify({ type: 'privkey' }),
      expect.any(Object),
    );
  });
  ...
  it('persists a reusable NIP-46 bunker session without an nsec', async () => {
    ...
    expect(SecureStore.setItemAsync).toHaveBeenCalledWith(
      'crays.identity.signer',
      JSON.stringify({ type: 'nip46', payload }),
      expect.any(Object),
    );
