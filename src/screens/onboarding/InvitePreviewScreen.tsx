  ...
  return <OnboardingShell testID="invite-preview-screen">
    ...
    {!loading && preview ? <>
      <PaperCard className="mt-7">
        <View className="flex-row items-start gap-4"><View className="h-12 w-12 items-center justify-center rounded-full bg-base-200"><Ionicons color={colors.accent} name="ticket-outline" size={25} /></View><View className="flex-1"><Text className="text-xs font-black uppercase tracking-[2px] text-base-300">Invited by</Text><Text className="mt-1 text-2xl font-black text-base-200">{room?.name || 'Verified membership issuer'}</Text><Text className="mt-2 text-base leading-6 text-base-300">{room?.about || 'The issuer is reachable. The signed room card is still loading.'}</Text></View></View>
