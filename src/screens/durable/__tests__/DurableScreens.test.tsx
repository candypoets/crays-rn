...
describe('durable screens', () => {
  ...
  it('keeps Me durable and routes access, history, recovery, and settings without live-room urgency', () => {
    const memberships = jest.fn(); const orders = jest.fn(); const profile = jest.fn(); const tickets = jest.fn();
    const view = render(<MeScreen accountState={{ status: 'ready', account }} hasMembership onMemberships={memberships} onOrders={orders} onProfile={profile} onTickets={tickets} ticketCount={1} />);
    expect(view.getByText('1 saved ticket')).toBeTruthy(); expect(view.queryByText('Ready for pickup')).toBeNull(); expect(view.queryByText(/current room/i)).toBeNull();
    expect(view.getByText(account.displayName)).toBeTruthy(); expect(view.getByText('Saved in Crays')).toBeTruthy();
    expect(view.getByText('Recovery & backup')).toBeTruthy(); expect(view.getByText('My access')).toBeTruthy(); expect(view.getByText('My history')).toBeTruthy();
    ['me-settings', 'me-recovery', 'me-orders', 'me-memberships', 'me-passes', 'me-tickets'].forEach((id) => fireEvent.press(view.getByTestId(id)));
    expect(profile).toHaveBeenCalledTimes(2); expect(orders).toHaveBeenCalledTimes(1); expect(memberships).toHaveBeenCalledTimes(2); expect(tickets).toHaveBeenCalledTimes(1);
  });
  ...
  it('renders honest empty durable context without inventing a room, active order, or wallet balance', () => { const view = render(<MeScreen hasMembership={false} onMemberships={jest.fn()} onOrders={jest.fn()} onProfile={jest.fn()} onTickets={jest.fn()} ticketCount={0} />); expect(view.queryByText('No room selected')).toBeNull(); expect(view.queryByTestId('me-current-room')).toBeNull(); expect(view.queryByTestId('me-active-order')).toBeNull(); expect(view.getByText('No saved tickets')).toBeTruthy(); expect(view.queryByText(/balance unavailable/)).toBeNull(); });
  ...
  it('does not turn an archived order venue into a current-room claim', () => { const view = render(<MeScreen activeOrder={{ ...order, roomName: 'Archive Room' }} hasMembership={false} onMemberships={jest.fn()} onOrders={jest.fn()} onProfile={jest.fn()} onTickets={jest.fn()} ticketCount={0} />); expect(view.queryByTestId('me-current-room')).toBeNull(); expect(view.queryByText('Ready for pickup')).toBeNull(); expect(view.queryByText('Archive Room')).toBeNull(); });
