    ...
    <AppShell testID="discover-screen">
      ...
      {loading ? (
        ...
        <><SectionTitle>{mode === 'nearby' ? 'Nearby now' : 'Room result'}</SectionTitle>
          <Pressable accessibilityLabel={`View ${room.name}`} className="overflow-hidden rounded-[24px] border border-base-300 bg-base-200 active:opacity-80" onPress={() => onOpenRoom ? onOpenRoom(room) : router.push({ pathname: '/room-preview' as never, params: { relay: room.relayUrl, room: room.id } } as never)} testID="room-result-card">
            <View className="h-32 justify-end bg-[#24151d] p-5"><Ionicons color={colors.accent} name="sparkles" size={32} /><Text className="mt-2 text-xs font-bold uppercase tracking-[3px] text-base-content">{room.name}</Text></View>
