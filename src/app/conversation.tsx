...
export default function ConversationRoute() {
  ...
  useEffect(() => {
    void loadLocalMessages().then((items) => {
  ...
  const deliver = async (messageType: 'message-acceptance' | 'message', content: string) => {
    ...
    try {
      ...
      const relays = Array.from(new Set(message.relayUrls?.length ? message.relayUrls : activeRoom ? [activeRoom.connectionRelayUrl || activeRoom.relayUrl] : []));
